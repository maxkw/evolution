from indirect_reciprocity import World, default_params,constant_stop_condition,prior_generator
from indirect_reciprocity import ReciprocalAgent, NiceReciprocalAgent, AltruisticAgent, SelfishAgent
...
def RA_v_AA(path = 'sims/RAvAA.pkl', overwrite = False):
    ...
    agent_types = [SelfishAgent, AltruisticAgent, NiceReciprocalAgent]
    ...
    for RA_prior in np.linspace(0.5, .9, 3):
        ...
        for r_id in range(N_runs):
            ...
            for a in [NiceReciprocalAgent, AltruisticAgent, SelfishAgent]:
                for oa in [1]:
                    for h in history:
                        data.append({
                            'round': h['round'],
                            'RA_prior': prior[NiceReciprocalAgent],
                            'belief': h['belief'][0][oa][a],
                            'type': a,
                            'oa':oa
                        })
...
def RA_v_AA_plot(in_path = 'sims/RAvAA.pkl',
    ...
    sns.factorplot('round', 'belief', hue='RA_prior', col='type',row='oa',data=df, ci=68)
    sns.despine()
    plt.ylim([0,1])
